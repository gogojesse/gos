...
char *put_dec(char *buf, unsigned long num)
{
	while (1) {
		unsigned rem;
		if (num < 100000)
			return put_dec_trunc(buf, num);
		rem = gos_do_div(num, 100000);
		//rem = num/100000;
 ...
 */
int num_to_str(char *buf, int size, unsigned long num)
{
...
gos_strnlen(const char *s, short max)
{
	size_t n;
 ...
 */
void gos_memcpy(char* dst, const char *src, int n)
{
	int i;
...
*/
		...
		case FORMAT_TYPE_NRCHARS: {
			unsigned char qualifier = spec.qualifier;
